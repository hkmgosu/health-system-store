 ...
 * @param {type} data
 * @param {type} options
 * @param {type} keepResponseErrors Se agrega nueva variable que permite forzar 
 * el envio de los errores cuando se esta en produccion.
 * @returns {unresolved}
 */
module.exports = function forbidden (data, options,keepResponseErrors) {
  ...
  if (sails.config.environment === 'production' && sails.config.keepResponseErrors 
          !== true && keepResponseErrors !== true) {
    data = undefined;
  }
  ...
  if (req.wantsJSON || sails.config.hooks.views === false) {
    return res.jsonx({ok: false, obj: data});
  }

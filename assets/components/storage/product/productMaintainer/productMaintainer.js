(() => {
     ...
     */
    app.directive('ssvqStorageProduct', function() {
        return {
            controller: ComponentController,
            controllerAs: 'vm',
            bindToController: {},
            restrict: 'E',
            templateUrl: '/components/storage/product/productMaintainer/productMaintainer.html'
        };
    });
    ...
    function ComponentController($scope, $mdDialog, $mdToast, $mdSidenav, $timeout, $location, productFactory, unitProductsManagerFactory) {
        ...
        unitProductsManagerFactory.getAllowedUnits().then(obj => {
            ...
            if (vm.parametricData.productTypes) {
                // asignar tipos de productos segun tipo de bodega asignada
                let filteredProductTypes = [];
                let allowedUnitsIsPharmaceuticalExist = _.find(vm.allowedUnits, {
                    isPharmaceutical: true
                });
                let allowedUnitsIsEconomatoExist = _.find(vm.allowedUnits, {
                    isPharmaceutical: false
                });
                vm.parametricData.productTypes.forEach(pType => {
                    if (allowedUnitsIsPharmaceuticalExist && pType.isPharmaceutical === true) {
                        console.log("pharma exist", pType);
                        filteredProductTypes.push(pType);
                    }
                    if (allowedUnitsIsEconomatoExist && pType.isPharmaceutical === false) {
                        console.log("economato exist", pType);
                        filteredProductTypes.push(pType);
                    }
                });

                vm.parametricData.productTypes = filteredProductTypes;
                console.log('filtered types', filteredProductTypes);
            }
        });
            ...
            $mdDialog.show({
                ...
                controller: function($mdDialog) {
                    ...
                    vm.onSelectProduct = product => {
                        if (product) {
                            let productTypeFound = _.find(vm.parametricData.productTypes, {
                                id: product.productType.id
                            });
                            if (!productTypeFound) {
                                $mdToast.showSimple("Producto seleccionado no cumple requisitos de tipo de bodega");
                                return;
                            }
                            vm.product = false;
                            ...
                            vm.productTypeSelected(product.productType.id);

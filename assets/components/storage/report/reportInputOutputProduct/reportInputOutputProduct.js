(function () {
    ...
    /**
     * Example
     * <ssvq-report-input-output-product key=""></ssvq-report-input-output-product-report>
     */
	app.directive('ssvqReportInputOutputProduct', () => {
		return {
			...
			restrict: 'E',
			scope: {},
			templateUrl: '/components/storage/report/reportInputOutputProduct/reportInputOutputProduct.html'
		};
	});
	...
	function ComponentController($mdSidenav, $mdDialog, $mdToast, storageReportFactory, unitProductsManagerFactory) {
		...
		vm.toggleUnit = unit => {
			...
			vm.loading = (vm.searchUnits.length > 0) ? false : true;
		...
		vm.getReportFilter = () => {
			...
			if (!vm.searchUnits.length > 0 || !vm.searchUnits) {
				vm.found = 0;
				vm.requests = [];
				return;
			}

			storageReportFactory.getInputOutputProduct({
		...
		vm.showCreatedAtFilter = ($event) => {
			$mdDialog.show({
				targetEvent: $event,
				templateUrl: '/components/storage/report/reportInputOutputProduct/rangeDate.html',
		...
		vm.getArrayXlsx = () => {
			...
			arr[0] = ['Id', 'Tipo Movimiento', 'Producto', 'Cod Producto', 'Lote', 'Unidad', 'Ubicación', 'Destino', 'Fecha', 'Expiración', 'Cantidad Entrada', 'Cantidad Salida', 'Stock Actual Lote'];
		...
		vm.generateXLSX = () => {
			...
			let filename = 'reporte_entrada_salida_por_producto_' + (moment().format('X')) + '.xlsx';
		...
		vm.getMargin = () => {
			...
			return (vm.searchUnits.length * 15) + 185;
		...
		vm.generatePdf = () => {
			return pdfMake.createPdf({
				...
				},
				{
					...
					table: {
						...
						body: [
							[{
								text: 'ENTRADA Y SALIDA POR PRODUCTO',
								style: 'column2'
							}]
						]
					},
				...
				content: [

					{
						...
						table: {
							headerRows: 1,
							widths: ['auto', '10%', '8%', '8%', '6%', '8%', '8%', '8%', '8%', '8%', '7%', '7%', '7%'],
			...
			}).download("reporte_entrada_salida_por_producto_" + (moment().format('X')) + ".pdf");
		};

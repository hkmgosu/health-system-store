(function () {
    ...
    /**
     * Example
     * <ssvq-report-most-requested-products key=""></ssvq-report-most-requested-products>
     */
	app.directive('ssvqReportMostRequestedProducts', () => {
		return {
			...
			restrict: 'E',
			scope: {},
			templateUrl: '/components/storage/report/reportMostRequestedProducts/reportMostRequestedProducts.html'
		};
	});
	...
	function ComponentController($mdSidenav, $mdDialog, $mdToast, storageReportFactory, productTypeFactory, unitProductsManagerFactory) {
		...
		vm.getReportInit = () => {
			storageReportFactory.getProductMostRequested({
		...
		vm.showCreatedAtFilter = ($event) => {
			$mdDialog.show({
				targetEvent: $event,
				templateUrl: '/components/storage/report/reportMostRequestedProducts/rangeDate.html',
		...
		vm.getArrayXlsx = () => {
			...
			arr[0] = ['Unidad', 'Tipo Producto', 'Producto', 'Stock'];
		...
		vm.generateXLSX = () => {
			...
			let filename = 'reporte_productos_mas_solicitados_' + (moment().format('X')) + '.xlsx';
		...
		vm.getMargin = () => {
			...
			return (vm.searchUnits.length * 15) + 190;
		...
		vm.generatePdf = () => {
			...
			}).download("reporte_productos_mas_solicitados_" + (moment().format('X')) + ".pdf");
		};

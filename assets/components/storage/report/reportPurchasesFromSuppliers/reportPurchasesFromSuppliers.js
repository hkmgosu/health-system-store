(function() {
    ...
    /**
     * Example
     * <ssvq-report-purchases-from-suppliers key=""></ssvq-report-purchases-from-suppliers>
     */
    app.directive('ssvqReportPurchasesFromSuppliers', () => {
        return {
            ...
            restrict: 'E',
            scope: {},
            templateUrl: '/components/storage/report/reportPurchasesFromSuppliers/reportPurchasesFromSuppliers.html'
        };
    });

    /* @ngInject */
    function ComponentController($mdSidenav, $mdDialog, $mdToast, storageReportFactory, productTypeFactory, unitProductsManagerFactory, companyFactory) {
        var vm = this;
        var vmParent = this;
        ...
        vm.colTitles = {
            ...
            'monto': 'Monto',
            'num_oc': 'Num OC',
            'num_donacion': 'Num Donación',
            'num_solicitud': 'Num Solicitud',
            'num_factura': 'Num Factura',
            'id_chile_compra': 'Chile Compra'
        };
        ...
        vm.getReportInit = () => {
            storageReportFactory.getPurchasesFromSuppliers({
        ...
        vm.getReportFilter = () => {
            storageReportFactory.getPurchasesFromSuppliers({
                ...
                company: vm.filter.company,
                product: vm.filter.product,
                document: vm.filter.searchText,
                minDate: vm.getDate(vm.filter.minDate),
                maxDate: vm.getDate(vm.filter.maxDate),
                page: vm.page,
                limit: vm.limit
        ...
        vm.showCreatedAtFilter = ($event) => {
            $mdDialog.show({
                targetEvent: $event,
                templateUrl: '/components/storage/report/reportPurchasesFromSuppliers/rangeDate.html',
        ...
        companyFactory.getAll().then(res => vm.companies = res.obj.companies);
        ...
        vm.generateXLSX = () => {
            ...
            let filename = 'reporte_compras_a_proveedores_' + (moment().format('X')) + '.xlsx';
        ...
        vm.getArrayFilter = () => {
            ...
            arr[count++] = vm.getBodyFilter('Proveedor: ', ((vm.filter.company ? _.find(vm.companies, {
                'id': vm.filter.company
            ...
            if (vm.filter.productName) {
                arr[count++] = vm.getBodyFilter('Producto: ', vm.filter.productName);
            }

            if (vm.filter.searchText) {
                arr[count++] = vm.getBodyFilter('Documento filtrado por: ', "'" + vm.filter.searchText + "'");
            }
        ...
        vm.generatePdf = () => {
            ...
            }).download("reporte_compras_a_proveedores_" + (moment().format('X')) + ".pdf");
        };
